...
export const BinaryGrid: React.FC<BinaryGridProps> = ({ size, data, highlightIndex, onHoverIndex, onToggle, label }) => {
  const gridStyle = {
    ...
    gridTemplateRows: `repeat(${size}, minmax(0, 1fr))`, // Force rows to match columns for perfect squares
      ...
      {data.map((val, idx) => (
        <div
          ...
          className={`
            ...
            ${/* Adjusted font sizes to ensure visibility at all supported resolutions (up to 64x64) */ ''}
