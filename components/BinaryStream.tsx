...
export const BinaryStream: React.FC<BinaryStreamProps> = ({ data, onHoverIndex, highlightIndex, onDataChange }) => {
    ...
    const handleSave = () => {
        ...
        onDataChange(cleaned.split('').map(Number));
        setIsEditing(false);
    };
    ...
    const handleChange = (e: React.ChangeEvent<HTMLTextAreaElement>) => {
        ...
        setInputValue(val);
        if (cleanInput(val).length === data.length) setError(null);
    };
    ...
    return (
        <div className={`bg-white text-slate-800 font-mono p-4 rounded-xl shadow-md w-full border-2 transition-colors ${isEditing ? 'border-blue-500 ring-2 ring-blue-500/20' : 'border-slate-200'}`}>
            <div className="flex justify-between items-center mb-3 border-b border-slate-100 pb-2">
                ...
                {!isEditing ? (
                    <button onClick={handleStartEdit} className="flex items-center gap-1 text-xs bg-slate-100 hover:bg-blue-600 hover:text-white px-2 py-1 rounded transition-colors text-slate-500">
                        <span className="text-[10px]">✏️</span> 修改编码
                    </button>
                ) : (
                    <div className="flex gap-2">
                        <button onClick={handleCancel} className="p-1 hover:bg-slate-100 rounded text-slate-400 hover:text-red-500 transition-colors" title="取消">
                            <XCircle size={16} />
                        </button>
                        <button onClick={handleSave} disabled={!isLengthCorrect} className={`flex items-center gap-1 px-3 py-1 rounded text-xs font-bold transition-all ${isLengthCorrect ? 'bg-green-600 text-white hover:bg-green-500 shadow-lg hover:scale-105' : 'bg-slate-200 text-slate-400 cursor-not-allowed'}`}>
                            <CheckCircle size={14} /> 生成图像
                        </button>
                    </div>
                )}
            </div>
            {isEditing ? (
                <div className="animate-in fade-in duration-200">
                    ...
                    {error && <div className="mt-2 text-red-500 text-xs flex items-center gap-1 animate-pulse"><AlertTriangle size={12} />{error}</div>}
                </div>
            ) : (
                <>
                        ...
                        {data.map((bit, idx) => (
                            <span
                                ...
                                className={`
                                    ...
                                    ${highlightIndex === idx ? 'bg-yellow-400 text-black font-bold scale-125 z-10' : (bit === 1 ? 'text-slate-900 font-extrabold bg-slate-100' : 'text-slate-300')}
                    ...
                    <div className="mt-2 flex justify-between items-end border-t border-slate-100 pt-2">
                        <div className="text-xs text-slate-500">* 只要改变这里的 0 和 1，上面的图片也会立刻变化哦！</div>
